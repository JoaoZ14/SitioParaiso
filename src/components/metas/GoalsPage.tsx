...
export default function GoalsPage() {
  ...
  const [metas, setMetas] = useLocalStorage<Meta[]>('sp-metas', []);
  const [financeGoal, setFinanceGoal] = useLocalStorage<FinanceMeta>('sp-metas-finance', { saved: 0, goal: 0 });
  ...
  const filteredMetas = useMemo(() => {
    ...
    return list.slice().sort((a, b) => {
      ...
      if (sortBy === 'progress') {
        // Ascending: show those with least progress first
  ...
  const handleToggleDone = (id: string) => {
    setMetas(prev =>
      prev.map(m => {
        if (m.id !== id) return m;
        const nowDone = m.status !== 'done';
        if (nowDone) {
          setToast({
            message: `"${m.title}" concluída!`,
            icon: <Sparkles className="w-4 h-4 text-yellow-300" />,
            key: Date.now(),
          });
        }
        return { ...m, status: nowDone ? 'done' : 'active', progress: nowDone ? 100 : m.progress };
      })
    );
  };

  const handleChecklistToggle = (metaId: string, itemId: string) =>
    setMetas(prev =>
      prev.map(m => {
        if (m.id !== metaId) return m;
        const updated = m.checklist.map(i =>
          i.id === itemId ? { ...i, done: !i.done } : i
        );
        // Auto-update progress based on checklist if no manual progress set
        const doneCount = updated.filter(i => i.done).length;
        const autoProgress = updated.length > 0
          ? Math.round((doneCount / updated.length) * 100)
          : m.progress;
        return { ...m, checklist: updated, progress: autoProgress };
      })
    );
  ...
  const handleCreateFromFinance = (title: string, goal: number) => {
    const meta: Meta = {
      id: genId(),
      title,
      category: 'Finanças',
  ...
  const handleExport = () => {
    ...
    setToast({ message: 'Metas exportadas com sucesso!', icon: <Download className="w-4 h-4 text-green-300" />, key: Date.now() });
  };

  // Clear all done metas
  const handleClearDone = () => {
    ...
    if (confirm(`Excluir ${doneCount} meta${doneCount > 1 ? 's' : ''} concluída${doneCount > 1 ? 's' : ''}?`)) {
      setMetas(prev => prev.filter(m => m.status !== 'done'));
      setShowTools(false);
    }
  };
